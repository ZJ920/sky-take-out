...
//        );
        ...
        //jsonObject.put("code", "ORDERPAID");//该订单已支付
    ...
    public PageResult getHistoryOrders(OrdersPageQueryDTO ordersPageQueryDTO) {
        ...
        //根据用户id查询订单
        Page<OrderVO> page = orderMapper.selectByUserId(userId);
        //获取总页数
        long total = page.getTotal();
        //获取订单列表
        List<OrderVO> historyOrders = page.getResult();
        //遍历订单列表，查询、添加订单详情
        for (OrderVO historyOrder : historyOrders) {
            Long orderId = historyOrder.getId();
            //加入订单详情
            historyOrder.setOrderDetailList(orderDetailMapper.selectByOrderId(orderId));
        }

        return new PageResult(total, historyOrders);
    }
